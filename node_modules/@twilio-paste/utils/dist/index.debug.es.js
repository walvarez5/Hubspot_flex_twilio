...
// src/hooks/useMutationObservable.ts
...
function useMutationObservable(targetEl, cb, options = DEFAULT_OPTIONS) {
  ...
  useEffect(() => {
    ...
    const hasDebounceTime = options.debounceTime && typeof options.debounceTime === "number" && options.debounceTime > 0;
    const obs = new MutationObserver(hasDebounceTime ? (0, import_debounce.default)(cb, options.debounceTime) : cb);
    setObserver(obs);
  }, [cb, options]);
  useEffect(() => {
    if (!observer)
...
function useWindowSize() {
  const [windowSize, setWindowSize] = useState2({
    width: void 0,
    height: void 0,
    breakpointIndex: void 0
  });
  const { breakpoints } = useTheme();
  const breakpointNumbers = breakpoints.map((bp) => Number.parseInt(bp.replace(/\D+/g, ""), 10));
  useEffect2(() => {
    const getBreakpointIndex = (windowWidth) => {
      const index = (0, import_findIndex.default)(breakpointNumbers, (bp) => windowWidth < bp);
      if (index === -1) {
        return breakpointNumbers.length - 1;
      }
      return index;
    };
    const handleResize = (0, import_debounce2.default)(() => {
      setWindowSize({
        width: window.innerWidth,
        height: window.innerHeight,
        breakpointIndex: getBreakpointIndex(window.innerWidth)
      });
    }, 50, { leading: false, trailing: true, maxWait: 150 });
